...
export class NotificationService {
	...
	static async notifyPostLike(
		...
		return NotificationService.createNotification({
			...
			metadata: { postId, likeUserId, postTitle },
	...
	static async notifyPostComment(
		...
		return NotificationService.createNotification({
			...
			type: "POST_COMMENT",
			title: "帖子收到评论",
			content: `${commentUserName} 评论了你的帖子 "${postTitle}"`,
			metadata: { postId, commentUserId, postTitle },
			actionUrl: `/posts/${postId}`,
			relatedUserId: commentUserId,
		});
	}
		...
		return NotificationService.createBulkNotifications(
			adminUserIds.map((adminUserId) => ({
				...
				actionUrl: organizationSlug
					? `/orgs/${organizationSlug}/settings/members`
					: `/orgs/${organizationId}/settings/members`,
				priority: "HIGH",

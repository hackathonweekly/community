...
} from "@community/lib-shared/auth/invitations";
import {
	ContentType,
...
import { withOrganizationPublicUrls } from "@community/lib-server/storage";
import { getAllTags, getBaseUrl } from "@community/lib-shared/utils";
import { validateProfileForOrganizationApplication } from "@community/lib-shared/utils/profile-validation";
...
const fetchOrganizationMembersWithUsers = async ({
	organizationId,
	userSelect,
	...
	for (const member of members) {
		...
		if (includeInviter && member.invitedBy) {
			userIds.add(member.invitedBy);
		}
	}

	const selectWithId: Prisma.UserSelect = {
		...userSelect,
		id: true,
	};
	const users =
	...
	return members.flatMap((member) => {
		...
		const inviterUser = member.invitedBy
			? (usersById.get(member.invitedBy) ?? null)
			: null;
...
export const organizationsRouter = new Hono()
	...
	.get(
		...
		async (c) => {
			...
			const members = await fetchOrganizationMembersWithUsers({
		...
		async (c) => {
			...
			try {
				...
				const normalizedCurrentUserEmail = currentUser.email
					...
					: "";
				const normalizedInvitationEmail = invitationRecord.email
				...
				if (
					...
					const existingMetadata =
						(invitationRecord.metadata as Prisma.JsonObject | null) ??
						{};
					const metadataRecord = existingMetadata as Record<
						string,
						Prisma.JsonValue
					>;
					...
					const updatedMetadata: Prisma.JsonObject = {
						...
						normalizedInvitationEmail
							? { originalEmail: normalizedInvitationEmail }
							: {}),
	...
	.post(
		...
		async (c) => {
			const { organizationId, invitationRequestCode, inviterId } =
				c.req.valid("json");
			const user = c.get("user");
			...
			try {
				...
				const resolvedInviterId =
					invitationRequest?.inviterId ?? inviterId ?? null;

				if (!resolvedInviterId) {
					return c.json({ error: "请填写邀请人" }, 400);
				}

				if (resolvedInviterId === user.id) {
					return c.json({ error: "邀请人不能是自己" }, 400);
				}

				const inviterMembership = await db.member.findFirst({
					where: {
						organizationId,
				...
				if (!inviterMembership) {
					return c.json({ error: "邀请人必须是该社区成员" }, 400);
				}

				const inviterProfile = await db.user.findUnique({
					where: { id: resolvedInviterId },
					select: {
						id: true,
						name: true,
						username: true,
				...
				const applicantProfile = await db.user.findUnique({
					where: { id: user.id },
					select: {
						id: true,
						name: true,
				...
				const profileValidation =
					validateProfileForOrganizationApplication(applicantProfile);

				if (!profileValidation.isComplete) {
					return c.json(
						{
							error: `资料未完善，请先完善：${profileValidation.missingFields.join("、")}`,
				...
				const formatProfileValue = (value?: string | null) =>
					value && value.trim().length > 0 ? value.trim() : "未填写";
				...
				const applicationReason = [
					...
					`【姓名】${formatProfileValue(applicantProfile.name)}`,
					`【用户名】${formatProfileValue(applicantProfile.username)}`,
					`【地区】${formatProfileValue(applicantProfile.region)}`,
					`【性别】${formatGenderLabel(applicantProfile.gender)}`,
					`【手机号】${formatProfileValue(applicantProfile.phoneNumber)}`,
					`【微信号】${formatProfileValue(applicantProfile.wechatId)}`,
					`【邮箱】${formatProfileValue(applicantProfile.email)}`,
					`【个人简介】${formatProfileValue(applicantProfile.bio)}`,
					`【主要角色】${formatProfileValue(applicantProfile.userRoleString)}`,
					`【当前在做】${formatProfileValue(applicantProfile.currentWorkOn)}`,
					`【当前状态】${formatProfileValue(applicantProfile.lifeStatus)}`,
				].join("\n\n");

				if (applicationReason.length > MAX_APPLICATION_REASON_LENGTH) {
					return c.json(
						{ error: "申请信息过长，请精简后再提交" },
						400,
					);
				}
						...
						if (admin.user?.email) {
							await sendEmail({
								...
								context: {
									...
									dashboardUrl: `${process.env.NEXT_PUBLIC_APP_URL}/orgs/${organization.slug}/settings/members`,
								},
							});
						}
					}
	...
	.post(
		...
		describeRoute({
			summary: "Create member invitation link",
		...
		async (c) => {
			...
			try {
				...
				const invitationPath = `/orgs/${organization.slug}/apply?inviteby=${encodeURIComponent(
					currentUser.id,
				)}`;

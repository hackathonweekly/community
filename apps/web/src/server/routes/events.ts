...
import {
	...
	getEventById,
	getEventPublicById,
	getEventSeriesForEventAssociation,
	getEventSeriesSubscribers,
	getEvents,
...
>;
type EventFindManyArgs = Parameters<typeof db.event.findMany>[0];
type EventWhereInput = NonNullable<EventFindManyArgs>["where"];
...
function generateEventSeriesUrl(seriesSlug: string) {
	const siteUrl = process.env.NEXT_PUBLIC_SITE_URL;
	const appUrl = process.env.NEXT_PUBLIC_APP_URL;
	const vercelUrl = process.env.VERCEL_URL;

	if (siteUrl && siteUrl.trim() !== "") {
		return `${siteUrl.trim().replace(/\/$/, "")}/events/series/${seriesSlug}`;
	}
	if (appUrl && appUrl.trim() !== "") {
		return `${appUrl.trim().replace(/\/$/, "")}/events/series/${seriesSlug}`;
	}
	if (vercelUrl && vercelUrl.trim() !== "") {
		return `https://${vercelUrl.trim().replace(/\/$/, "")}/events/series/${seriesSlug}`;
	}

	return `/events/series/${seriesSlug}`;
}

async function validateSeriesAssociationForEvent(params: {
	...
	const series = await getEventSeriesForEventAssociation(normalizedSeriesId);
	if (!series) {
		return {
			ok: false as const,
	...
	if (!series.isActive) {
		return {
			ok: false as const,
			error: "Series is archived and cannot be linked",
		};
	}
	...
	if (organizationId) {
		if (series.organizationId !== organizationId) {
			return {
				ok: false as const,
				error: "Series host mismatch: organization events can only link to series owned by the same organization",
			};
		}
	} else {
		if (series.organizationId) {
			return {
				ok: false as const,
				error: "Series host mismatch: personal events cannot link to organization-owned series",
			};
		}
		if (series.organizerId !== params.organizerId) {
			return {
				ok: false as const,
				error: "Series host mismatch: personal events can only link to series owned by the same organizer",
			};
		}
	}
...
async function notifySeriesSubscribersOfNewEvent(params: {
	event: Event;
	seriesId?: string | null;
	excludeUserIds?: string[];
}) {
	try {
		...
		const series =
			await getEventSeriesForEventAssociation(normalizedSeriesId);
		if (!series || !series.slug) {
			return;
		}

		const subscribers = await getEventSeriesSubscribers(normalizedSeriesId);
		if (subscribers.length === 0) {
			return;
		}
		...
		const filteredSubscribers = subscribers.filter(
			(subscriber) => !excludedIds.has(subscriber.userId),
		);
		if (filteredSubscribers.length === 0) {
			return;
		}

		const eventLocationDisplay = formatEventLocationDisplay(params.event);
		const seriesUrl = generateEventSeriesUrl(series.slug);

		const inAppTargets = filteredSubscribers.filter(
			(subscriber) => subscriber.notifyInApp,
		);
		if (inAppTargets.length > 0) {
			await NotificationService.createBulkNotifications(
				inAppTargets.map((subscriber) => ({
					userId: subscriber.userId,
					type: "EVENT_REMINDER",
					title: `系列活动更新：${series.title}`,
					content: `${params.event.title} 已发布，点击查看系列活动详情`,
					actionUrl: `/events/${params.event.shortId || params.event.id}`,
					metadata: {
						seriesId: series.id,
		...
		const emailTargets = filteredSubscribers.filter(
			(subscriber) =>
				subscriber.notifyEmail &&
				subscriber.canReceiveEmail &&
				isSendableEmail(subscriber.email),
		);
		...
		await Promise.allSettled(
			emailTargets.map((subscriber) => {
				const locale = inferLocaleCode(subscriber.locale);
				const eventDate = formatEventDateForLocale(
					params.event.startTime,
					locale,
				);
				const eventLocation =
					locale === "en"
						...
						: eventLocationDisplay.zh;
				const eventUrl = generateEventUrl(
					params.event.shortId || params.event.id,
					locale,
				);

				return sendEventSeriesNewEventAnnouncement({
		...
		console.error(
			"Failed to notify event series subscribers about new event:",
			error,
		);
	}
}
...
function formatLocalizedMessages(
	locale: Locale | undefined,
	messages: LocalizedMessage[],
): string {
	const isEnglish = locale === "en";
	return messages
		.map((message) => (isEnglish ? message.en : message.zh))
		.join("\n\n");
}
...
const updateEventSchema = z.object({
	...
	type: z.enum(["MEETUP", "HACKATHON"]).optional(),
	status: z
		.enum([
			"DRAFT",
			"PUBLISHED",
...
async function getEventWithAdminCheck(eventId: string, headers: Headers) {
	...
	const event = isEventAdmin
		...
		: await getEventPublicById(eventId);
...
app.post("/", async (c) => {
	try {
		...
		const cleanedData = {
			...
			organizationId:
				data.organizationId === null || data.organizationId === ""
					...
					: data.organizationId,
			seriesId:
				data.seriesId === null || data.seriesId === ""
					? undefined
					: data.seriesId,
		};
		...
		const seriesValidation = await validateSeriesAssociationForEvent({
			seriesId: restEventData.seriesId,
			organizerId: actingUserId,
			organizationId: restEventData.organizationId,
		});
		...
		const event = await createEvent({
			...
			hackathonConfig:
				restEventData.type === "HACKATHON" && normalizedHackathonConfig
					? (normalizedHackathonConfig as any)
					: undefined,
		});
		...
		if (event.status === "PUBLISHED") {
			...
			await notifySeriesSubscribersOfNewEvent({
				event,
		...
		const eventUrl = generateEventUrl(event.shortId || event.id);
		return c.json({
			success: true,
			data: {
				...event,
...
app.put("/:id", zValidator("json", updateEventSchema), async (c) => {
	try {
		...
		const nextOrganizationId =
			restUpdateData.organizationId !== undefined
				? restUpdateData.organizationId
				: existingEvent.organizationId;
		const nextSeriesId =
			restUpdateData.seriesId !== undefined
				? restUpdateData.seriesId
		...
		if (shouldSendUpdateEmail) {
			try {
				...
				if (recipients.length > 0) {
					const updateMessageParts: LocalizedMessage[] = [];
					...
					if (updateMessageParts.length > 0) {
						...
						await Promise.all(
							recipients.map((recipient) => {
								...
								const eventUrl = generateEventUrl(
									updatedEvent.shortId || updatedEvent.id,
									locale,
								);
								const updateDetails = formatLocalizedMessages(
									locale,
									updateMessageParts,
								);
			...
			if (registrations.length > 0) {
				...
				try {
					...
					if (recipients.length > 0) {
						const eventLocationDisplay =
							formatEventLocationDisplay(existingEvent);
						const updateMessage = buildBilingualMessage(
							"活动已取消，请留意后续通知。",
							"The event has been cancelled. Please stay tuned for updates.",
						);
						...
						await Promise.all(
							recipients.map((recipient) => {
								...
								const eventUrl = generateEventUrl(
									existingEvent.shortId || existingEvent.id,
									locale,
								);
								const updateDetails = formatLocalizedMessages(
									locale,

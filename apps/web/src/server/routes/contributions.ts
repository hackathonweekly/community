...
import { getOrganizationMembership } from "@community/lib-server/database";
import {
	hasPermission,
	AdminPermission,
...
export const contributionsRouter = new Hono()
	...
	.get("/", async (c) => {
		try {
			...
			let levelInfo = null;
			if (fullUser) {
				const cpValue = fullUser.cpValue || 0;
				const currentLevel = calculateMembershipLevel(cpValue);
				const currentLevelInfo = getLevelInfo(
					"membership",
					currentLevel,
				);
				const nextLevelInfo = getNextLevelInfo(currentLevel);
				const progress = calculateLevelProgress(cpValue, currentLevel);

				levelInfo = {
					currentLevel: currentLevel,
					levelName: currentLevelInfo.label,
					currentLevelCp: LEVEL_THRESHOLDS[currentLevel],
					nextLevel: nextLevelInfo?.level || null,
					nextLevelName: nextLevelInfo?.label || null,
					nextLevelCp: nextLevelInfo?.threshold || null,

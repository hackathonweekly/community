...
import {
	...
	CollapsibleContent,
	CollapsibleTrigger,
} from "@community/ui/ui/collapsible";
...
export function SubmissionsSlideDeck({
	...
	const currentSubmissionDetailUrl = useMemo(() => {
		...
		const normalizedBase = baseUrl.endsWith("/")
			? baseUrl.slice(0, -1)
			: baseUrl;
		return `${normalizedBase}/events/${eventId}/submissions/${currentSubmission.id}`;
	}, [baseUrl, eventId, currentSubmission]);
	...
	const currentSubmissionVoteUrl = useMemo(() => {
		...
		const normalizedBase = baseUrl.endsWith("/")
			? baseUrl.slice(0, -1)
			: baseUrl;
		return `${normalizedBase}/events/${eventId}/submissions/${currentSubmission.id}/vote`;
	}, [baseUrl, eventId, currentSubmission]);

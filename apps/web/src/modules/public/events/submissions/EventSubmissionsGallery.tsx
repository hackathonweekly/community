...
import { ToggleGroup, ToggleGroupItem } from "@community/ui/ui/toggle-group";
import {
	useEventSubmissions,
	useUnvoteSubmission,
	useVoteSubmission,
...
export function EventSubmissionsGallery({
	...
	const registerHref = `/events/${eventId}/register`;
	...
	const renderVoteButton = (submission: EventSubmission) => {
		...
		if (isOwnTeam) {
			return (
				...
				>
					<Link
						href={`/events/${eventId}/submissions/${submission.id}/edit`}
						title="自己的作品，不能投票"
					>
				...
				<div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
					...
					<div className="flex flex-wrap items-center gap-2 md:justify-end">
						...
						<Select value={sortValue} onValueChange={setSortValue}>
							<SelectTrigger className="h-11 w-full sm:max-w-[220px]">
							...
							return (
								...
								>
									<CardContent className="p-3">
										<div className="flex items-center gap-3">
											<Link
												href={`/events/${submission.eventId}/submissions/${submission.id}`}

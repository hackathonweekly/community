...
export default function EventEditPage() {
	...
	const fetchEventSeriesOptions = async () => {
		try {
			const response = await fetch(
				"/api/event-series?mine=true&limit=100",
			);
			if (!response.ok) {
	...
	if (loading || organizationsLoading || eventSeriesLoading) {
		return (
	...
	const seriesIdValue = event.seriesId ?? "none";
	...
	return (
		<>
			<MobilePageHeader
				title={t("editEvent")}
				onBack={() => router.push(`/events/${eventId}/manage`)}
				rightAction={

...
A live audit on 2026-07-16 found 1,789 objects using 16.818 GiB in `hackweek-public-1303088253`. MP4/MOV files alone account for 228 objects and about 89.8% of all bytes. A database-reference audit identified 297 abandoned submission objects using 11.431 GiB; these do not need to move. The migration must stop serving and writing active objects through Tencent COS without losing referenced content or leaving old absolute COS URLs in production data.
...
- Keep low-traffic videos as ordinary R2 objects. With abandoned submissions excluded, the expected R2 footprint is about 5.387 GiB, below the current 10 GB Standard free tier; managed video storage or transcoding would cost more and add failure modes without reducing the bill.

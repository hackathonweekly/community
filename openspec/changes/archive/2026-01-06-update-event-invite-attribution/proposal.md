...
- Affected code（实现阶段会涉及的关键位置）：
  - 前端：`apps/web/src/app/(public)/[locale]/events/[eventId]/register/EventRegistrationPage.tsx`
  - 前端：`apps/web/src/app/(public)/[locale]/events/[eventId]/hooks/useUnifiedEventRegistration.ts`
  - 前端：`apps/web/src/app/(public)/[locale]/events/[eventId]/components/EventLayout.tsx`（现有缓存逻辑对齐）
  - 后端：`src/server/routes/events/registrations.ts`（`inviteCode -> inviteId` 写入点）
  - 统计：`packages/lib-server/src/database/prisma/queries/event-invites.ts`

...
export type AdminInsights = PublicInsights & {
	...
	segments: {
		...
		creatorLevel: InsightsBucket[];
		mentorLevel: InsightsBucket[];
		contributorLevel: InsightsBucket[];
...
async function computeAdminInsightsLight(): Promise<AdminInsights> {
	...
	const creatorLevel = toBuckets(
		creatorLevelGroup.map((row) => ({
			key: toDisplayKey(row.creatorLevel, "未设置"),
			count: row._count._all,
		})),
		totalUsers,
	);

	const mentorLevel = toBuckets(
		mentorLevelGroup.map((row) => ({
			key: toDisplayKey(row.mentorLevel, "未设置"),
			count: row._count._all,
		})),
		totalUsers,
	);

	const contributorLevel = toBuckets(
		contributorLevelGroup.map((row) => ({
			key: toDisplayKey(row.contributorLevel, "未设置"),
			count: row._count._all,
		})),
		totalUsers,
	);

...
import {
	type OAuthProvider,
	oAuthProviders,
} from "@dashboard/auth/constants/oauth-providers";
import { useUserAccountsQuery } from "@dashboard/auth/lib/api";
...
export function ConnectedAccountsBlock() {
	...
	const isProviderLinked = (provider: OAuthProvider) => {
		...
		return data?.some((account) => account.provider === provider);
	};

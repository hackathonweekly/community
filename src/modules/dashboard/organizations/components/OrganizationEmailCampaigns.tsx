...
export function OrganizationEmailCampaigns() {
	...
	const fetchCampaigns = async () => {
		try {
			...
			const response = await fetch(
				`/api/organizations/${organizationSlug}/admin/emails/campaigns?${params}`,
			);
			if (response.ok) {
	...
	const sendCampaign = async (campaignId: string) => {
		try {
			const response = await fetch(
				`/api/organizations/${organizationSlug}/admin/emails/campaigns/${campaignId}/send`,
				{
					method: "POST",
				},
			);
			if (response.ok) {
				const data = await response.json();
				alert(
					`活动发送已启动，预计发送给 ${data.recipientCount} 个组织成员`,
				);
				fetchCampaigns();
			}
		} catch (error) {
	...
	const deleteCampaign = async (campaignId: string) => {
		...
		try {
			const response = await fetch(
				`/api/organizations/${organizationSlug}/admin/emails/campaigns/${campaignId}`,
				{
					method: "DELETE",
				},
			);
	...
	return (
		<div className="p-6 space-y-6">
			...
			<div className="flex items-center justify-between">
				<div>
					<h1 className="text-3xl font-bold">组织邮件活动管理</h1>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>管理您的组织邮件发送活动</CardDescription>
				</CardHeader>
				<CardContent>
					{/* 搜索和筛选 */}

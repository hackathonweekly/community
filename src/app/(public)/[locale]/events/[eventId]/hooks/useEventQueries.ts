...
import { cacheInvalidation, requestDeduplicator } from "@/lib/cache-config";
	...
	const cancelRegistrationMutation = useMutation({
		mutationFn: async () => {
			...
			if (!response.ok) {
				...
				throw new Error(
					error.error || t("events.registrationCancelFailed"),
				);
			}
	...
	const volunteerApplyMutation = useMutation({
		mutationFn: async (eventVolunteerRoleId: string) => {
			...
			if (!response.ok) {
				const error = await response.json();
				throw new Error(
					error.error || t("events.volunteerApplicationFailed"),
				);
			}
...
export function useEventProjectSubmissions(eventId: string) {

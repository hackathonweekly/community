...
export default function AwardsCeremonyPage({
		...
		const fetchData = async () => {
			try {
				...
				const response = await fetch(
					`/api/event-submissions?eventId=${eventId}&sort=voteCount&includeVotes=true`,
				);
	...
	useEffect(() => {
		if (!autoPlay || topThree.length === 0) return;
				...
				<div className="w-full max-w-7xl mx-auto">
					...
					<div className="text-center mb-12 space-y-6 animate-fade-in">
						...
						<div className="flex justify-center mb-6">
							<div
								className={cn(
									"p-8 rounded-full bg-gradient-to-br shadow-2xl",
						...
						<div className="space-y-4">
							<Badge
								className={cn(
									"text-lg px-6 py-2",
									getLevelBadgeColor(currentWinner.rank),
								)}
							>
								...
								<div className="flex items-start gap-3 text-white/90">
									...
									<div className="space-y-2">
										...
										{currentWinner.teamMembers &&
											currentWinner.teamMembers.length >
												0 && (
												<p className="text-lg">
													<span className="text-white/60">
														队员：

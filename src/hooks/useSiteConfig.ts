...
import type { ContactFormConfig, CustomerServiceConfig } from "@/config/types";
...
export function useCustomerServiceConfig(
	initial: CustomerServiceConfig = appConfig.customerService,
): ConfigState<CustomerServiceConfig> {
	const [state, setState] = useState<ConfigState<CustomerServiceConfig>>({
		data: initial,
		loading: true,
		error: null,
	});
	...
	useEffect(() => {
		let isMounted = true;
		fetchConfig<CustomerServiceConfig>("/api/site-config/customer-service")
			.then((remoteConfig) => {
				if (!isMounted) {
					return;
				}

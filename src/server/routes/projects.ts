...
import { checkAndAwardAutoBadges } from "@/lib/database/prisma/queries/badges";
import {
	CP_VALUES,
	recordContribution,
...
import { getCachedOrganizations } from "@/lib/cache/organizations";
	...
	.delete("/projects/:id", async (c) => {
		try {
			...
			await db.$transaction(async (tx) => {
				// Delete related BuildingRegistration records first
				await tx.buildingRegistration.deleteMany({
					where: { projectId },
				});

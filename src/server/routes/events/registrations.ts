...
import { sendEventReviewNotificationSMS } from "@/lib/sms/tencent-sms";
		...
		try {
			...
			if (event.type === "HACKATHON") {
				const hackathonStage =
					...
					"REGISTRATION";
				if (hackathonStage !== "REGISTRATION") {
					return c.json(
						{
							success: false,
							error: "Registration is not open at this stage",
						},
						400,
					);
				}
...
app.get(
	...
	async (c) => {
		try {
			...
			const result = await getEventRegistrations(eventId, params);
...
app.delete(
	...
	async (c) => {
		try {
			...
			// Check if registration exists
			const registration = await getEventRegistration(eventId, userId);
			if (!registration) {
				return c.json(
					{
						success: false,
			...
			if (needsSMSNotification && updatedRegistration.user.phoneNumber) {
				try {
					...
					if (smsResult.success) {
						console.log(
							`SMS notification sent successfully to ${updatedRegistration.user.phoneNumber} for event ${event.title}`,
						);
					} else {
						console.warn(
	...
	async (c) => {
		try {
			...
			const headers = [
				...
				"能提供什么/Can Offer",
				"在寻找什么/Looking For",
				"成员等级/Membership Level",
				"创造者等级/Creator Level",
				"导师等级/Mentor Level",
				"贡献者等级/Contributor Level",
				"CP积分/CP Value",
				"票种名称/Ticket Type",
				"票种价格/Ticket Price",
			...
			for (const registration of registrations) {
				...
				const row = [
					...
					escapeCsvValue(registration.user.creatorLevel),
					escapeCsvValue(registration.user.mentorLevel),
					escapeCsvValue(registration.user.contributorLevel),

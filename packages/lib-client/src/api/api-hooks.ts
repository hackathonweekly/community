...
import {
	archiveEventSeries,
	createEventSeries,
	fetchEventSeriesSubscriptionStatus,
	fetchEventsList,
	...
	type EventSeriesMutationPayload,
	type EventSeriesSubscriptionState,
	type EventSeriesSummary,
	type EventListItem,
	type EventOrganizationSummary,
} from "@community/lib-shared/api/api-fetchers";
import { queryKeys } from "@community/lib-shared/query-keys";
...
export function useEventSeriesListQuery(
	params?: {
		page?: number;
		limit?: number;
		search?: string;
	...
	return useQuery<{
		...
		queryKey: queryKeys.events.series.list(params),
		queryFn: () => fetchEventSeriesList(params),
		...cacheConfig.realtime,
		enabled: options?.enabled ?? true,
	});
}

export function useEventSeriesDetailQuery(
	identifier?: string,
	options?: { enabled?: boolean },
) {
	return useQuery<EventSeriesDetail | null>({
		queryKey: queryKeys.events.series.detail(identifier || ""),
		queryFn: () => fetchEventSeriesDetail(identifier || ""),
		...cacheConfig.realtime,
		enabled: Boolean(identifier) && (options?.enabled ?? true),
	});
}

export function useEventSeriesSubscriptionQuery(
	identifier?: string,
	options?: { enabled?: boolean; userId?: string },
) {
	return useQuery<EventSeriesSubscriptionState>({
		queryKey: queryKeys.events.series.subscription(
			identifier || "",
			options?.userId,
		),
		queryFn: () => fetchEventSeriesSubscriptionStatus(identifier || ""),
		...cacheConfig.realtime,
		enabled: Boolean(identifier) && (options?.enabled ?? true),
	});
}

export function useCreateEventSeriesMutation() {
	const queryClient = useQueryClient();

	return useMutation({
		mutationFn: (payload: EventSeriesMutationPayload) =>
			createEventSeries(payload),
		onSuccess: () => {
			queryClient.invalidateQueries({
				queryKey: queryKeys.events.series.list(),
			});
		},
	});
}

export function useUpdateEventSeriesMutation() {
	const queryClient = useQueryClient();
	...
	return useMutation({
		...
		}) => updateEventSeries(params.identifier, params.payload),
		onSuccess: (_, variables) => {
			queryClient.invalidateQueries({
				queryKey: queryKeys.events.series.list(),
			});
			queryClient.invalidateQueries({
				queryKey: queryKeys.events.series.detail(variables.identifier),
			});
		},
	});
}

export function useArchiveEventSeriesMutation() {
	const queryClient = useQueryClient();

	return useMutation({
		mutationFn: (identifier: string) => archiveEventSeries(identifier),
...
export function useSubscribeEventSeriesMutation() {
	const queryClient = useQueryClient();
	...
	return useMutation({
		mutationFn: (params: {
			...
			userId?: string;
		}) => subscribeEventSeries(params.identifier, params.payload),
		onSuccess: (_, variables) => {
			queryClient.invalidateQueries({
				queryKey: queryKeys.events.series.subscription(
...
export function useUnsubscribeEventSeriesMutation() {
	const queryClient = useQueryClient();

	return useMutation({
		mutationFn: (params: { identifier: string; userId?: string }) =>
			unsubscribeEventSeries(params.identifier),
		onSuccess: (_, variables) => {
			queryClient.invalidateQueries({
				queryKey: queryKeys.events.series.subscription(
					variables.identifier,

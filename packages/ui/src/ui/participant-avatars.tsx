...
import type { DigitalBusinessCardData } from "@community/ui/ui/digital-business-card";
import { DigitalBusinessCardGallery } from "@community/ui/ui/digital-business-card-gallery";
...
export function ParticipantAvatars({
	...
	useEffect(() => {
		...
		setSearchTerm("");
		setCurrentPage(1);
	}, [dialogOpen, requireAuth, currentUserId, onRequireAuth, setDialogOpen]);
				...
				{displayAvatars.map((participant, index) => (
					...
					>
						<UserAvatar
							...
							className={cn(
								"h-8 w-8 border ring-2",
								darkBackground
									? "border-white/10 ring-white/20"
									: "border-gray-200 ring-white",
							)}
						/>
					</div>
				))}
									...
									onClick={() => {
										const returnTo =
											typeof window !== "undefined"
												...
												: "";
										const profilePath = `/u/${participant.user.username || participant.user.id}`;
								...
								>
									...
									<div className="min-w-0 flex-1">
										...
										<div className="truncate text-xs text-muted-foreground">
											...
											{participant.user.userRoleString &&
												participant.user
													.currentWorkOn && (
													<span>{" · "}</span>
												)}
											{participant.user.currentWorkOn && (
												<span>
													{
														participant.user
															.currentWorkOn

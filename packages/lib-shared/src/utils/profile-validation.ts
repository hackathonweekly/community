...
const REQUIRED_ORGANIZATION_APPLICATION_FIELDS: ProfileRequirementDefinition[] =
	[
		{
			key: "name",
			label: "姓名",
		...
		},
		{
			...
			label: "用户名",
			sectionId: "essential-info",
			isSatisfied: (user) => stringFilled(user.username),
		},
		{
			key: "region",
			label: "地区",
			sectionId: "essential-info",
			isSatisfied: (user) => stringFilled(user.region),
		},
		{
			key: "gender",
			label: "性别",
			sectionId: "essential-info",
			isSatisfied: (user) => stringFilled(user.gender),
		},
		{
			key: "phoneNumber",
			label: "手机号",
...
function buildProfileValidation(
	user: UserForValidation,
	requiredDefinitions: ProfileRequirementDefinition[] = REQUIRED_PROFILE_FIELDS,
	recommendedDefinitions: ProfileRequirementDefinition[] = RECOMMENDED_PROFILE_FIELDS,
): UserProfileValidation {
	const requiredFields = evaluateRequirements(user, requiredDefinitions);
...
/**
 * 检查用户是否具备申请组织的基本条件
 * 必填字段包含：姓名、用户名、地区、性别、手机号、微信号、邮箱、个人简介、主要角色、当前在做、当前状态
 */
export function validateProfileForOrganizationApplication(
	user: UserForValidation,
): UserProfileValidation {
	return buildProfileValidation(
		user,
		REQUIRED_ORGANIZATION_APPLICATION_FIELDS,
	);
}

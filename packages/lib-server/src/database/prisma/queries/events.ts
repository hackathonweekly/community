...
import { getPublicStorageUrl } from "@community/lib-server/storage";
...
const OCCUPYING_REGISTRATION_STATUSES = ["PENDING", "APPROVED"] as const;

const REGISTRATION_ALREADY_CANCELLED_ERROR =
	"Registration is already cancelled or does not require cancellation";

function createTicketSoldOutError(ticketName?: string) {
	return new Error(
		ticketName
			? `Ticket type "${ticketName}" is sold out`
			: "Ticket type is sold out",
	);
}

export function isOccupyingRegistrationStatus(
	status: RegistrationStatus | null | undefined,
): status is RegistrationStatus {
	return Boolean(
		status &&
			OCCUPYING_REGISTRATION_STATUSES.includes(
				status as (typeof OCCUPYING_REGISTRATION_STATUSES)[number],
			),
	);
}

async function releaseTicketInventory(
...
async function reserveTicketInventory(
	...
	const { ticketTypeId, ticketName } = params;
	let snapshot = params.initialSnapshot;

	for (let attempt = 0; attempt < 3; attempt++) {
		if (!snapshot || snapshot.maxQuantity === null) {
			const unlimitedResult = await tx.eventTicketType.updateMany({
				where: {
					id: ticketTypeId,
					isActive: true,
				},
			...
			if (unlimitedResult.count > 0) {
				return;
			}

			throw createTicketSoldOutError(ticketName);
		}

		if (snapshot.currentQuantity >= snapshot.maxQuantity) {
			throw createTicketSoldOutError(ticketName);
		}

		const reserveResult = await tx.eventTicketType.updateMany({
			where: {
				id: ticketTypeId,
				isActive: true,
		...
		const latestTicket = await tx.eventTicketType.findUnique({
		...
		if (
			latestTicket.maxQuantity !== null &&
			latestTicket.currentQuantity >= latestTicket.maxQuantity
		) {
			throw createTicketSoldOutError(ticketName);
		}

		snapshot = {
			currentQuantity: latestTicket.currentQuantity,
			maxQuantity: latestTicket.maxQuantity,
		};
	}
...
/**
 * Resolve an event identifier that could be either a shortId or a CUID.
 * CUIDs are 25 chars starting with 'c'; shortIds are 8 chars.
 * Returns a Prisma where clause.
 */
export function resolveEventIdentifier(identifier: string) {
...
const eventByIdPublicInclude = Prisma.validator<Prisma.EventInclude>()({
...
type EventByIdPublic = Prisma.EventGetPayload<{
	include: typeof eventByIdPublicInclude;
}>;
...
export async function getEventById(
	...
	const where = resolveEventIdentifier(id);
...
export async function getEventPublicById(
	id: string,
): Promise<EventByIdPublic | null> {
	const event = await db.event.findUnique({
		where: resolveEventIdentifier(id),
		include: eventByIdPublicInclude,
	});
...
export async function getEventMetadataById(id: string) {
...
export async function updateEventRegistration(
	...
	databaseClient: Pick<typeof db, "$transaction"> = db,
) {
	return await databaseClient.$transaction(async (tx) => {
		...
		const nextStatus = data.status ?? existingRegistration.status;
		const wasOccupying = isOccupyingRegistrationStatus(
			existingRegistration.status,
		);
		const willOccupy = isOccupyingRegistrationStatus(nextStatus);

		if (existingRegistration.ticketTypeId && wasOccupying !== willOccupy) {
			if (wasOccupying && !willOccupy) {
				await releaseTicketInventory(
					tx,
					existingRegistration.ticketTypeId,
				);
			} else if (!wasOccupying && willOccupy) {
				await reserveTicketInventory(tx, {
					ticketTypeId: existingRegistration.ticketTypeId,
					ticketName: existingRegistration.ticketType?.name,
					initialSnapshot: existingRegistration.ticketType
						? {
								currentQuantity:
									existingRegistration.ticketType
										.currentQuantity,
								maxQuantity:
									existingRegistration.ticketType.maxQuantity,
							}
						: undefined,
				});
			}
...
export async function deleteEventRegistration(
	eventId: string,
	userId: string,
	databaseClient: Pick<typeof db, "eventRegistration" | "$transaction"> = db,
) {
	const registration = await databaseClient.eventRegistration.findUnique({
		where: {
			eventId_userId: {
				eventId,
				userId,
	...
	if (
		...
		throw new Error(REGISTRATION_ALREADY_CANCELLED_ERROR);
	}
	...
	return await databaseClient.$transaction(async (tx) => {
		...
		if (
			...
			isOccupyingRegistrationStatus(registration.status)
		) {
			await releaseTicketInventory(tx, registration.ticketTypeId);
		}
...
export async function registerForEvent(data: {
	...
	type RegistrationPayload = Prisma.EventRegistrationGetPayload<{
		include: {
			event: {
				select: {
					id: true;
	...
	if (event.ticketTypes.length > 0) {
		...
		if (
			...
			throw createTicketSoldOutError(selectedTicket.name);
		}
	...
	return await db.$transaction(async (tx) => {
		...
		if (validTicketTypeId) {
			await reserveTicketInventory(tx, {
				...
				initialSnapshot: selectedTicket
					? {
							currentQuantity: selectedTicket.currentQuantity,
							maxQuantity: selectedTicket.maxQuantity,
						}
					: undefined,
			});
		}

...
async function checkOpenAI() {
	try {
		...
		if (!apiKey) {
			logResult("OpenAI API", null, "未配置 API Key，跳过测试", "connections");
			return true;
		}
		...
		// Test OpenAI API connection with a simple chat completion request
		...
		if (response.ok) {
			...
			if (data.choices && data.choices.length > 0) {
				logResult("OpenAI API", true, `连接成功，模型 ${model} 可用`, "connections");
			} else {
				logResult("OpenAI API", false, `API 响应异常，未收到预期格式`, "connections");
			}
		} else {
			...
			if (response.status === 401) {
				logResult("OpenAI API", false, "API Key 无效或过期", "connections");
			} else if (response.status === 403) {
				logResult("OpenAI API", false, "API Key 权限不足或配额不足", "connections");
			} else if (response.status === 404) {
				const originalBaseUrl = process.env.OPENAI_BASE_URL || "https://api.openai.com/v1";
				logResult("OpenAI API", false, `端点不存在，请检查 OPENAI_BASE_URL 配置 (当前: ${originalBaseUrl})`, "connections");
			} else if (response.status === 422) {
				logResult("OpenAI API", false, `模型 "${model}" 不支持或参数错误`, "connections");
			} else {
				logResult("OpenAI API", false, errorMessage, "connections");
			}
		}
		...
		if (error.code === 'ENOTFOUND' || error.code === 'ECONNREFUSED') {
			const originalBaseUrl = process.env.OPENAI_BASE_URL || "https://api.openai.com/v1";
			logResult("OpenAI API", false, `无法连接到服务器，请检查 OPENAI_BASE_URL (${originalBaseUrl})`, "connections");
		} else if (error.message.includes('fetch')) {
			logResult("OpenAI API", false, `网络请求失败: ${error.message}`, "connections");
		} else {
			logResult("OpenAI API", false, error.message, "connections");
		}
		return false;
	}
}
...
async function main() {
	...
	if (results.failed > 0) {
		...
		if (results.details.some(r => !r.status && r.test.includes("OpenAI"))) {
			console.log("   6. 验证 OpenAI API Key 是否有效");
		}
	} else {

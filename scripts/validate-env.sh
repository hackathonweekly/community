...
main() {
    ...
    case "$action" in
        ...
        "compare")
            # 比较测试和生产环境
            local env1="${2:-.env.test}"
            local env2="${3:-.env.production}"
            local name1="${4:-测试环境}"
            local name2="${5:-生产环境}"
        ...
        "sync")
            # 生成同步建议
            local source="${2:-.env.test}"
            local target="${3:-.env.production}"
        ...
        *)
            ...
            echo "示例:"
            echo "  $0 validate"
            echo "  $0 compare .env.test .env.production 测试环境 生产环境"
            echo "  $0 docs"
            exit 1
            ;;
    esac
...
示例:
    $0 validate                                    # 验证所有环境
    $0 compare .env.test .env.production           # 比较测试和生产环境
    $0 docs                                       # 生成文档
    $0 sync .env.test .env.production              # 生成同步建议
